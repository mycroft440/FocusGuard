...
internal object WebsiteBlockingDiagnostics {
    ...
    fun beginTransition(
        ...
        val matchedRule = blockedCandidate
            ?.let { WebsiteBlocker.findMatchingRule(it, blockedRules) }
    ...
    fun listReports(): List<WebsiteBlockingDiagnosticReport> {
        val directory = reportDirectory() ?: return emptyList()
    ...
    fun readReport(fileName: String): String? {
        if (!validReportName(fileName)) return null
        val directory = reportDirectory() ?: return null
    ...
    private fun writeFailureAsync(
        ...
        scope.launch {
            runCatching {
                val directory = reportDirectory() ?: return@runCatching
                ...
                val report = buildString {
                    ...
                    appendLine()
                    appendLine(
                        "Privacidade: este relatório não grava query string, fragmento, conteúdo da página, " +
                            "senhas ou texto digitado em formulários."
                    )
                }
                ...
                val browser = state.browserPackageName
                    ...
                    .take(80)
                val suffix = if (state.transitionId > 0L) "_t${state.transitionId}" else ""
                File(directory, "$REPORT_PREFIX${stamp}_${browser}${suffix}$REPORT_SUFFIX")
    ...
    private fun cleanOldReports() {
        val directory = reportDirectory() ?: return
